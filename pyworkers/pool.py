        ...
        try:
            ...
            def next_inputs():
                ...
                try:
                    ...
                    logger.debug('At least one input sequence has been deplated - no more data will be enqueued to any worker')
                    self._deplated = True
                    return None
            ...
            def enqueue(worker):
                ...
                if not self._deplated:
                    ...
                    logger.debug('Input deplated - closing {}', worker)
                    #worker.close()
            ...
            while self._pending and set(self._get_all_workers_ids()).difference(self._finished):
                ...
                for conn in ready:
                    ...
                    try:
                        ...
                        logger.debug('EOFError occured while reading from a pipe: {} - will try to issue artificial closing message', conn)
                        found = False
