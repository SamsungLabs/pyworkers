...
class PersistentContextTest(GenericTest):
    ...
    def test_good(self):
        self.context = RemoteContext(1, target=test_fun, host=self.server_addr, kwargs={ 'exp': 3 })
    ...
    def test_terminate_context(self):
        self.context = RemoteContext(1, target=long_test_fun, host=self.server_addr, kwargs={ 'exp': 3 })
        self.worker = PersistentRemoteWorker(None, host=self.context.host, context=self.context.context_id)
        self.assertTrue(self.worker.is_alive())
